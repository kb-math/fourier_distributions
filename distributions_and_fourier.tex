...
\begin{eg} The Fourier-Laplace transform of a Gaussian $u(x) = C e^{- \alpha x^2}$, where $C,\alpha>0$, must be an entire function (holomorphic everywhere) on $\bC$. Let us compute it. Assume for convenience that $u(x) = C e^{-x^2}$ where $C>0$ is chosen so that $$\int_{\bR} u(x) dx = 1.$$ Now 
