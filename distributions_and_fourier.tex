...
\subsection{Various properties of the Fourier Transform}

\begin{prop} If $\mu$ and $\nu$ are finite Borel measures on $\bR^d$, then the convolution $\mu * \nu$ is a well defined finite Borel measure given by $$(\mu * \nu) (A) = (\mu \times \nu) (\Sigma^{-1} A) \quad\text{for Borel } A \subset \bR^d$$ where $\sigma:\bR^d \times \bR^d \to \bR^d$ is the summation map $\sigma(x,y) = x+y$. We have that $$\cF( \mu \times \nu) = \cF(\mu) \cF(\nu).$$
